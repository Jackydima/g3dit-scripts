...
public class ScriptUpdateEntityPropertiesScript implements IScript {
	...
	public boolean execute(IScriptEnvironment env) {
		return ScriptUtils.processAndSaveWorldFiles(env, (archive, file) -> {
			...
			SearchFilter<eCEntity> filter = env.getSearchPanel().buildFilter();
			if (!filter.isValid()) {
				return 0;
			}
			...
			for (eCEntity entity : archive.getEntities()) {
				...
				if (filter.matches(entity) && entity.hasClass(CD.gCNPC_PS.class) && entity.hasClass(CD.gCInventory_PS.class)) {
	...
	@Override
	public boolean entitySearchEnabled() {
		return true;
	}
}